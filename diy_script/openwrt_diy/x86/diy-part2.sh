...
sed -i 's/\${host:-openwrt.org}/www.qq.com/g' feeds/luci/modules/luci-mod-network/root/etc/uci-defaults/50_luci-mod-admin-full
...
# ddnsto
clone_dir https://github.com/linkease/ddnsto-openwrt-package luci-app-ddnsto ddnsto
...
# 更改 argon主题设置 顺序
sed -i 's/"order"[[:space:]]*:[[:space:]]*85/"order": 90/g' $destination_dir/luci-app-argon-config/root/usr/share/luci/menu.d/luci-app-argon-config.json
...
if [ "${REPO_BRANCH#*-}" = "23.05" ]; then
   sed -i "s/DISTRIB_DESCRIPTION=.*/DISTRIB_DESCRIPTION=\"%D %V (By @Jejz build $(TZ=UTC-8 date '+%Y-%m-%d') $(TZ=UTC-8 date +'%H' | sed -e 's/4/5/g'):$(TZ=UTC-8 date +%M | sed -e 's/4/5/g'))\"/g" package/base-files/files/etc/openwrt_release
   echo -e "\e[41m当前写入的编译时间:\e[0m \e[33m$(grep 'DISTRIB_DESCRIPTION' package/base-files/files/etc/openwrt_release)\e[0m"
else
   sed -i "s/DISTRIB_DESCRIPTION=.*/DISTRIB_DESCRIPTION=\"%D %V By @Jejz\"/g"  package/base-files/files/etc/openwrt_release
   sed -i "s/OPENWRT_RELEASE=.*/OPENWRT_RELEASE=\"%D %V (By @Jejz build $(TZ=UTC-8 date '+%Y-%m-%d') $(TZ=UTC-8 date +'%H' | sed -e 's/4/5/g'):$(TZ=UTC-8 date +%M | sed -e 's/4/5/g'))\"/g" package/base-files/files/usr/lib/os-release
